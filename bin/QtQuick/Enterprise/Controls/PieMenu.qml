...
/*!
    ...
    they will not hidden. By default, the boundingItem is set to the parent of
    the menu. It can also be set to \c null to prevent this behavior.
    ...
    \l {QtQuick.Enterprise.Controls.Styles::PieMenuStyle::}{startAngle} or
    \l {QtQuick.Enterprise.Controls.Styles::PieMenuStyle::}{endAngle}; the
    remaining items will grow to consume the available space.

    You can create a custom appearance for a PieMenu by assigning a
    \l {QtQuick.Enterprise.Controls.Styles::}{PieMenuStyle}.
*/
    ...
    property QtObject __protectedScope: QtObject {
        ...
        */
        function isMouseOver(itemIndex) {
            ...
            if (sectionStart > Utils.pi2) {
                sectionStart %= Utils.pi2;
            } else if (sectionStart < -Utils.pi2) {
                sectionStart %= -Utils.pi2;
            }

            if (sectionEnd > Utils.pi2) {
                sectionEnd %= Utils.pi2;
            } else if (sectionEnd < -Utils.pi2) {
                sectionEnd %= -Utils.pi2;
            }
        ...
        */
        ...
        readonly property real startAngleRadians: Utils.degToRadOffset(__style.startAngle)
        readonly property real endAngleRadians: Utils.degToRadOffset(__style.endAngle)
